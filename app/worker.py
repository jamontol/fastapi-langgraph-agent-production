"""Async worker that consumes tasks from the Valkey stream and publishes replies via pub/sub."""

import asyncio
import json
from app.core.langgraph.graph import LangGraphAgent
from app.core.logging import logger
from app.core.valkey import CONSUMER_GROUP, STREAM_KEY, init_valkey_stream, valkey_client
...
async def process_stream_task(task_id: str, payload: dict):
    ...
    try:
        ...
        async for chunk in agent.get_stream_response(messages, session_id, user_id=user_id, username=username):
            await valkey_client.publish(channel_name, json.dumps({"content": chunk, "done": False}))
        ...
        await valkey_client.publish(channel_name, json.dumps({"error": str(e), "done": True}))
...
async def process_sync_task(task_id: str, payload: dict):
    ...
    try:
        ...
        result = await agent.get_response(messages, session_id, user_id=user_id, username=username)
        await valkey_client.publish(channel_name, json.dumps({"messages": result}))
    except Exception as e:
...
async def start_worker():
    """Run the worker loop, consuming tasks from the Valkey stream until interrupted."""
